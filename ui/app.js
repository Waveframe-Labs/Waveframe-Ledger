...
function publishCurrentBundleToRegistry(receipt, publicationNotes) {
  ...
  const lifecycle = registryLifecycleEvents(existing).length
    ...
    : [
        ...
        lifecycleEvent("reviewed", now, { bundle_hash: bundle.immutable_inputs.preview_hash }, "Semantic artifacts generated by Ledger."),
      ];
...
function lifecycleEventDetail(event) {
  return event.notes?.detail || event.detail || "Lifecycle event recorded.";
}
...
function authorityRegistryCard(entry) {
  ...
  appendMeta(meta, "Registered", formatDateTime(entry.published_at || entry.updated_at));
  ...
  appendMeta(meta, "Bundle hash", entry.latest_bundle_hash || entry.contract_hash);
  ...
  for (const event of registryLifecycleEvents(entry)) {
    ...
    eventDetail.textContent = `${formatDateTime(event.timestamp)} | ${lifecycleEventHash(event)}`;
