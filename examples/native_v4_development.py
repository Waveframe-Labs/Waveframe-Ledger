"""Generate complete development fixtures using the installed candidate compiler.

Requires WAVEFRAME_LEDGER_ACTION_POLICY_DEV=1 and the pinned development dependency.
Writes fixture artifacts only; performs no repository file operations or activation.
"""
...
def build_fixture(name: str, rows=None) -> dict:
    ...
    identity = dict(source_policy_id="repository-policy", source_revision="revision-1",
                    authority_id="repository-" + name, authority_version="2.0.0")
    draft = interpret_policy_with_domain_pack(source, domain_pack_id="repository-changes",
        domain_pack_version="2.0.0", **identity)
    ...
    for statement, (_, specs) in zip(draft["source_statements"], rows):
        ...
        clauses.append({"start_byte": start, "end_byte": end, "coverage_status": "fully_represented",
            "candidate_controls": controls, "unresolved_binding_ids": [], "limitation_code": None,
    ...
    run = create_policy_translation_run(
        ...
        provider_identifier="development-fixture-author", translation_template_version="fixture-v1",
        ...
        created_at="2026-09-11T12:00:00Z", completed_at="2026-09-11T12:00:01Z",
        sequence_number=0, previous_run_hash=None, catalog_version="2.0.0")
    proposal = create_policy_translation_proposal(source, **identity, clauses=clauses,
        organizational_bindings=[], translation_runs=[run], catalog_version="2.0.0")
    confirmation = None
    for clause in proposal["clauses"]:
        for control in clause["candidate_controls"]:
            confirmation = apply_policy_translation_control_confirmation(proposal, confirmation,
                ...
                confirmed_by="policy-owner", confirmed_at="2026-09-11T12:01:00Z")
        confirmation = apply_policy_translation_disposition(proposal, confirmation,
            clause_id=clause["clause_id"], coverage_status="fully_represented",
            reason_code="human-confirmed-complete", confirmed_by="policy-owner",
            confirmed_at="2026-09-11T12:02:00Z")
    ...
    approval = approve_policy_translation_proposal(proposal, confirmation,
        approved_by="policy-owner", approved_at="2026-09-11T12:03:00Z")
    publication = finalize_policy_translation_authority_v4(proposal, confirmation, approval,
        committed_by="ledger-committer", committed_at="2026-09-11T12:04:00Z",
        publication_id="development-" + name, published_by="ledger-publisher",
        published_at="2026-09-11T12:05:00Z")
...
if __name__ == "__main__":
    root = Path("tests/fixtures/action_policy_v4")
    for name in POLICIES:
        folder = root / name
        folder.mkdir(parents=True, exist_ok=True)
        for artifact, value in build_fixture(name).items():
    ...
    print("Wrote create-only, modify-only and mixed development artifacts.")
