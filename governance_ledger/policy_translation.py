...
def get_policy_translation_capability_catalog(*, catalog_version: str = "1.0.0") -> dict[str, Any]:
    """Select an exact installed catalog; the rollout default remains historical 1.0.0."""
    return _catalog_for_version(catalog_version)
...
def resolve_policy_translation_capability_catalog(
    ...
    if key in {(_CATALOG_ID, "2.0.0"), (_CATALOG_ID, "3.0.0")}:
        from governance_ledger.action_policy import get_action_capability_catalog
        catalog = get_action_capability_catalog(catalog_ref["catalog_version"])
        if catalog_ref != {key: catalog[key] for key in ("catalog_id", "catalog_version", "catalog_hash")}:
            raise ValueError("registered action capability catalog hash is unavailable")
        return catalog
...
def validate_policy_translation_capability_catalog(catalog: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(catalog, dict) and catalog.get("schema_version") == "policy_translation_capability_catalog.v2":
        from governance_ledger.action_policy import get_action_capability_catalog
        if catalog != get_action_capability_catalog(catalog.get("catalog_version")):
            raise ValueError("action catalog is not the installed immutable catalog")
...
def validate_policy_translation_proposal(proposal: dict[str, Any]) -> dict[str, Any]:
    ...
    for index, (clause, statement) in enumerate(zip(clauses, expected_statements)):
        ...
        if catalog["catalog_version"] in {"2.0.0", "3.0.0"} and statement["classification"] != "direct" and clause["candidate_controls"]:
...
def _control_selections(control: dict[str, Any], resolutions: dict[str, str]) -> tuple[str, dict[str, Any]]:
    ...
    if control["enforcement_point"] in {"waveframe.guard.repository-change.v2-development", "waveframe.guard.repository-change.v2"}:
...
def _direct_statement_semantics(draft: dict[str, Any], statement_id: str) -> list[dict[str, Any]]:
    ...
    return [
        _direct_constraint_semantics(constraints[constraint_id], action_aware=draft["domain_pack"]["domain_pack_version"] in {"2.0.0", "3.0.0"})
...
def _render_control(control: dict[str, Any], resolutions: dict[str, str]) -> str:
    if control["enforcement_point"] in {"waveframe.guard.repository-change.v2-development", "waveframe.guard.repository-change.v2"}:
        from governance_ledger.action_policy import REVIEW_BOUNDARY, RELEASE_REVIEW_BOUNDARY
        value = control["value"]
        ...
        boundary = REVIEW_BOUNDARY if control["enforcement_point"].endswith("-development") else RELEASE_REVIEW_BOUNDARY
